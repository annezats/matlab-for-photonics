    ...
    if size(cellfun('length', sample(i).material),2) == 1
        ...
        while(k<count*repeat)
            ...
            while(b < count)
                ...
                layer(iln+b+k).N =interp1(sample(i).lambda{1,b+1}, sample(i).N{1,b+1},lambda_a);
                %disp (iln+j+k)
                layer(iln+b+k).thickness = sample(i).thickness{1,b+1};
                b=b+1;
            end
            k=k+b;
        end
...
%         T01P = ((1/fm(4))*[1 fm(3); fm(3) 1])*T01P;
        ...
        Re_S(i,b) = real(rS(i)*conj(rS(i)));
        Re_P(i,b) = real(rP(i)*conj(rP(i)));

        % Power Transmission Coefficient
        nreal_f = real(layer(n_layers).N(i));
        nreal_i = real(layer(1).N(i));
        Tr_S(i,b) = real((nreal_f/nreal_i)*(cos(thetaN)/cos(theta0))*tS(i)*conj(tS(i)));
        Tr_P(i,b) = real((nreal_f/nreal_i)*(cos(thetaN)/cos(theta0))*tP(i)*conj(tP(i)));
    
        % Power Absorption Coefficient
        Abs_S(i,b) = 1 - Re_S(i,b) - Tr_S(i,b);
        Abs_P(i,b) = 1 - Re_P(i,b) - Tr_P(i,b);
