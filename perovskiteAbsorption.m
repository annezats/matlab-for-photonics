...
sample(2).material = {'2D DJ PEROVSKITE'};
%thickness is in the loop
sample(2).repeat = 1;
sample(2).lambda = wl*10^-9;
sample(2).N = nt+i*kt; %nt+i*kt;

% % substrate layer
sample(3).material = {'glass'};
sample(3).repeat = 1;
sample(3).lambda = (M.glass.wavelength)*10^-9;
sample(3).N = M.glass.n + (M.glass.k).*1i;
...
for c=1:1:2
    ...
    perovskiteinfo(c).thickness= cw*10^-9;
    perovskiteinfo(c).percent=change(c)*100;
    perovskiteinfo(c).A=Abs_S;
    
    f=1;
    if c>1
        while f<=imax
            A0=perovskiteinfo(c-1).A(f);
            Aprime=perovskiteinfo(c).A(f);
            dA(f)=(Aprime-A0);
            f=f+1;
        end
        perovskiteinfo(c).dA=dA;
    end
